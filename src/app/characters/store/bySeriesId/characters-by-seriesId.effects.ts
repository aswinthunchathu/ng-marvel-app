...
export class CharactersBySeriesIdEffects {
    private _URL = (action: fromCharactersByComicIdActions.type) =>
        `series/${action['payload']}/characters?orderBy=-modified`

    /*
     * This effect is fired when FETCH_CHARACTERS_BY_SERIES_ID_START action is fired
     */
    @Effect() fetchCharacters = this.actions$.pipe(
        ofType(fromCharactersByComicIdActions.FETCH_CHARACTERS_BY_SERIES_ID_START),
    ...
    @Effect() fetchCharactersNextPage = this.actions$.pipe(
        ofType(fromCharactersByComicIdActions.FETCH_CHARACTERS_BY_SERIES_ID_NEXT_PAGE),
        withLatestFrom(this.store.select('charactersBySeriesId')),
        switchMap(([action, characterState]) => {
            const pagination: Pagination = characterState.pagination

            if (!pagination.hasMore) {
                return of({ type: fromCharactersByComicIdActions.NO_MORE_CHARACTERS_BY_SERIES_ID })
            } else {
    ...
    private _fetchComics(
        ...
        | fromCharactersByComicIdActions.FetchCharactersBySeriesIdSuccess
        | fromCharactersByComicIdActions.FetchCharactersBySeriesIdError
    > {
        return this.http$
            ...
            .pipe(
                ...
                map(
                    res =>
                        new fromCharactersByComicIdActions.FetchCharactersBySeriesIdSuccess(
                            res.results.map(
                                item =>
                                    new CharacterModel(
                                        item.id,
                ...
                catchError(err => of(new fromCharactersByComicIdActions.FetchCharactersBySeriesIdError(err)))
            )
    }
}
